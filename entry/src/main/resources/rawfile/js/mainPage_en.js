...
for (let i = 0;i < allGridItem.length; i++) {
  allGridItem[i].innerHTML = chargeAmount[i] + 'CNY';
...
function changeVal() {
  ...
  if (name !== 'Phone Number' && input.length < 13) {
    document.getElementById('phone_tip').innerHTML = 'Phone Number';
  }
  let result = input;
...
function recharge() {
  ...
  return document.getElementById('phone').value.length === 13 ? 'Recharge successful' : 'Please enter right number';
}